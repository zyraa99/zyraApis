  ...
  const fetchApi = async (url, name, method = 'GET') => {
    ...
    try {
      ...
      // ── JSON ──
        ...
        const videoVal = flatVals.find(v => isVideoUrl(v));
        const imageVal = !videoVal && flatVals.find(v => isImageUrl(v));

        if (videoVal) {
          // Render JSON dulu, lalu preview video di bawah
          renderJsonResponse(data);
          renderVideoPreview(videoVal, name);
          showHelper(videoVal, method, false, true);
        } else if (imageVal) {
          renderJsonResponse(data);
          const img = document.createElement('img');
