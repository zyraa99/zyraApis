document.addEventListener('DOMContentLoaded', async () => {
  ...
  const updateNotifBadge = () => {
    ...
    DOM.notifBadge?.classList.toggle('active', unread.length > 0);
  };
  ...
  const populatePage = () => {
    ...
    setText(DOM.pageTitle, settings.name, 'Zyrä API');
  ...
  const detectMethod = (item) => {
    ...
    // Heuristic: kalau ada params body atau method dideklarasi
    return 'GET';
  };
  ...
  const renderCategories = () => {
    ...
    settings.categories.forEach((cat, ci) => {
      ...
      if (cat.icon) header.innerHTML = `<i class="${cat.icon}"></i>`;
      ...
      sorted.forEach((item, ii) => {
        ...
        card.innerHTML = `
          ...
          <div class="card-path">${pathClean}</div>
  ...
  const handleSearch = () => {
    ...
    const show = total === 0 && term.length > 0;
  ...
  const resetModal = () => {
    ...
    if (helperContainer) {
      ...
      const firstTab = helperContainer.querySelector('.helper-tab');
      const firstPanel = helperContainer.querySelector('.helper-panel');
  ...
  const setupModal = (api) => {
    ...
    // Parse params from path query string
  ...
  const buildParamForm = (keys, api) => {
    ...
    keys.forEach(key => {
      ...
      // Cek tipe param: bisa 'text' | 'textarea' | { type: 'select', options: [...] }
      ...
      if (typeInfo && typeof typeInfo === 'object' && typeInfo.type === 'select') {
        ...
        hidden.type = 'hidden';
        hidden.id = `param-${key}`;
        hidden.dataset.param = key;
        hidden.required = !isOptional;
        ...
        ta.id = `param-${key}`;
        ta.placeholder = hint || `Masukkan ${key}...`;
        ta.dataset.param = key;
        ta.required = !isOptional;
        ta.rows = 4;
        ta.autocomplete = 'off';
        ...
        input.id = `param-${key}`;
        input.placeholder = hint || `Masukkan ${key}...`;
  ...
  const validateInputs = () => {
    // Ambil semua required fields: input, select, textarea
    ...
    fields?.forEach(f => { if (f.value.trim()) f.classList.remove('is-invalid'); });
  };
  ...
  // ─── FETCH API ───
  const fetchApi = async (url, name, method = 'GET') => {
    ...
    try {
      ...
      if (DOM.respStatus) {
        ...
        DOM.respStatus.className = `status-pill ${ok ? 'ok' : 'err'}`;
      }
      ...
      if (ct.includes('image/')) {
        ...
        // Tampilin total count di status badge kalau ada
        ...
        if (DOM.respContent) {
          DOM.respContent.textContent = text || '(Respons kosong)';
      ...
      showHelper(url, method, ct.includes('image/'));
      ...
      if (DOM.respStatus) {
        DOM.respStatus.textContent = 'Error';
        DOM.respStatus.className = 'status-pill err';
      }
      if (DOM.respContainer) DOM.respContainer.classList.remove('hidden');
  ...
  // ─── HELPER GENERATOR ───
  const showHelper = (url, method = 'GET', isImage = false) => {
    const helperContainer = document.getElementById('helperContainer');
    if (!helperContainer) return;
    const encUrl = url.replace(/'/g, "\'");
    const fetchCode = isImage
      ? `const res = await (async () => {\n  const r = await fetch('${encUrl}');\n  const buf = Buffer.from(await r.arrayBuffer());\n  return buf;\n})();\n// res is a Buffer — bisa langsung kirim ke chat`
      : method === 'POST'
        ? `const res = await (async () => {\n  const r = await fetch('${encUrl}', {\n    method: 'POST',\n    headers: { 'Content-Type': 'application/json' }\n  });\n  return r.json();\n})();\nconsole.log(res);`
        : `const res = await (async () => {\n  const r = await fetch('${encUrl}');\n  return r.json();\n})();\nconsole.log(res);`;
    const axiosCode = isImage
      ? `const res = await (async () => {\n  const { default: axios } = await import('axios');\n  const { data } = await axios.get('${encUrl}', { responseType: 'arraybuffer' });\n  return Buffer.from(data);\n})();\n// res is a Buffer — bisa langsung kirim ke chat`
      : method === 'POST'
        ? `const res = await (async () => {\n  const { default: axios } = await import('axios');\n  const { data } = await axios.post('${encUrl}');\n  return data;\n})();\nconsole.log(res);`
        : `const res = await (async () => {\n  const { default: axios } = await import('axios');\n  const { data } = await axios.get('${encUrl}');\n  return data;\n})();\nconsole.log(res);`;
    const curlCode = isImage
      ? `curl -s '${url}' --output image.png`
      : method === 'POST'
        ? `curl -X POST '${url}' \\\n  -H 'Content-Type: application/json' \\\n  -H 'Accept: application/json'`
        : `curl -s -X GET '${url}' \\\n  -H 'Accept: application/json'`;
    const codes = { fetch: fetchCode, axios: axiosCode, curl: curlCode };
    const escHtml = s => s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    const panels = { fetch: 'helperFetch', axios: 'helperAxios', curl: 'helperCurl' };
    Object.entries(panels).forEach(([k, id]) => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = `<pre class="helper-code">${escHtml(codes[k])}</pre>`;
    });
    helperContainer.classList.remove('hidden');
    // Reset ke tab pertama (fetch) setiap kali showHelper dipanggil
    helperContainer.querySelectorAll('.helper-tab').forEach(t => t.classList.remove('active'));
    helperContainer.querySelectorAll('.helper-panel').forEach(p => p.classList.remove('active'));
  ...
  const setupEvents = () => {
    ...
    DOM.copyEndpoint?.addEventListener('click', () => {
      copyText(DOM.apiEndpoint?.textContent || '', DOM.copyEndpoint);
    });
    DOM.copyResponse?.addEventListener('click', () => {
      copyText(DOM.respContent?.textContent || '', DOM.copyResponse);
    });
  ...
  (() => {
    ...
    const mp             = gid('musicPlayer');
    const mpFull         = gid('mpFull');
    const mpBar          = gid('mpBar');
    const mpExpandBtn    = gid('mpExpandBtn');
    const mpCollapseBtn  = gid('mpCollapseBtn');
    const mpPlayPause    = gid('mpPlayPause');
    const mpPlayPauseBar = gid('mpPlayPauseBar');
    const mpPlayIcon     = gid('mpPlayIcon');
    const mpPlayIconBar  = gid('mpPlayIconBar');
    const mpThumb        = gid('mpThumb');
    const mpBarThumb     = gid('mpBarThumb');
    const mpTitle        = gid('mpTitle');
    const mpBarTitle     = gid('mpBarTitle');
    const mpBarSub       = gid('mpBarSub');
    const mpDur          = gid('mpDur');
    const mpSeekFill     = gid('mpSeekFill');
    const mpSeekThumb    = gid('mpSeekThumb');
    const mpSeekTrack    = gid('mpSeekTrack');
    const mpVolSlider    = gid('mpVolSlider');
    const mpArtworkGlow  = gid('mpArtworkGlow');
    const mpSearchInput  = gid('mpSearchInput');
    const mpSearchBtn    = gid('mpSearchBtn');
    const mpSearchResult = gid('mpSearchResult');
    const mpSearchLoading= gid('mpSearchLoading');
    const mpSearchError  = gid('mpSearchError');
    const mpResultThumb  = gid('mpResultThumb');
    const mpResultTitle  = gid('mpResultTitle');
    const mpResultDur    = gid('mpResultDur');
    const mpPlayResult   = gid('mpPlayResult');
    ...
    const setPlayIcon = playing => {
      ...
      if (mpPlayIcon) mpPlayIcon.className = `fas ${icon}`;
      if (mpPlayIconBar) mpPlayIconBar.className = `fas ${icon}`;
      mp.classList.toggle('playing', playing);
    };

    const loadTrack = data => {
      audio.src = data.mp3;
      audio.load();
      [mpThumb, mpBarThumb].forEach(el => { if (el) { el.src = data.thumbnail || ''; }});
      if (mpTitle) mpTitle.textContent = data.title;
      if (mpBarTitle) mpBarTitle.textContent = data.title;
      if (mpBarSub) mpBarSub.textContent = data.duration || '';
      if (mpDur) mpDur.textContent = `0:00 / ${data.duration || '0:00'}`;
      if (mpSearchResult) mpSearchResult.classList.add('hidden');
    ...
    mpPlayPauseBar?.addEventListener('click', togglePlay);
    mpExpandBtn?.addEventListener('click', () => { mpFull?.classList.remove('hidden'); mpBar?.classList.add('hidden'); });
    mpCollapseBtn?.addEventListener('click', () => { mpFull?.classList.add('hidden'); mpBar?.classList.remove('hidden'); });

    audio.addEventListener('timeupdate', () => {
      if (!audio.duration || isSeeking) return;
      const pct = (audio.currentTime / audio.duration) * 100;
      if (mpSeekFill) mpSeekFill.style.width = `${pct}%`;
      if (mpSeekThumb) mpSeekThumb.style.left = `${pct}%`;
      if (mpDur) mpDur.textContent = `${fmtTime(audio.currentTime)} / ${fmtTime(audio.duration)}`;
    });

    const seekTo = e => {
      if (!audio.duration || !mpSeekTrack) return;
      const rect = mpSeekTrack.getBoundingClientRect();
      const pct = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
      audio.currentTime = pct * audio.duration;
      if (mpSeekFill) mpSeekFill.style.width = `${pct*100}%`;
      if (mpSeekThumb) mpSeekThumb.style.left = `${pct*100}%`;
    };
    mpSeekTrack?.addEventListener('click', seekTo);
    mpSeekTrack?.addEventListener('mousedown', e => {
      isSeeking = true; seekTo(e);
      const up = () => { isSeeking = false; window.removeEventListener('mouseup', up); };
      window.addEventListener('mouseup', up);
    });

    mpVolSlider?.addEventListener('input', e => { audio.volume = e.target.value / 100; });
    gid('mpSkipBack')?.addEventListener('click', () => { audio.currentTime = Math.max(0, audio.currentTime - 10); });
    gid('mpSkipFwd')?.addEventListener('click', () => { audio.currentTime = Math.min(audio.duration || 0, audio.currentTime + 10); });
    ...
    const doSearch = async () => {
      ...
      if (!q) return;
      if (mpSearchResult) mpSearchResult.classList.add('hidden');
      if (mpSearchError) mpSearchError.classList.add('hidden');
      if (mpSearchLoading) mpSearchLoading.classList.remove('hidden');
      if (mpSearchBtn) mpSearchBtn.disabled = true;
      try {
        const r = await fetch(`https://api-varhad.my.id/search/ytplay?q=${encodeURIComponent(q)}`);
        const json = await r.json();
        if (!json.status || !json.result?.mp3) throw new Error(json.message || 'Lagu tidak ditemukan');
        lastResult = json.result;
        if (mpResultThumb) mpResultThumb.src = lastResult.thumbnail || '';
        if (mpResultTitle) mpResultTitle.textContent = lastResult.title;
        if (mpResultDur) mpResultDur.textContent = lastResult.duration || '';
        if (mpSearchResult) mpSearchResult.classList.remove('hidden');
      } catch (err) {
        if (mpSearchError) { mpSearchError.textContent = err.message; mpSearchError.classList.remove('hidden'); }
      } finally {
        if (mpSearchLoading) mpSearchLoading.classList.add('hidden');
        if (mpSearchBtn) mpSearchBtn.disabled = false;
      }
    };

    mpSearchBtn?.addEventListener('click', doSearch);
    mpSearchInput?.addEventListener('keydown', e => { if (e.key === 'Enter') doSearch(); });
    mpPlayResult?.addEventListener('click', () => { if (lastResult) loadTrack(lastResult); });
  })();
